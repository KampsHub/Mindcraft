...
The instructional designer lens: What is the learning objective? What does the learner do to get there? How do they know they got there? Does the sequence build understanding before asking for application? Is the difficulty appropriate? Does the exercise create genuine insight or just busywork? This lens applies to all exercise content, coaching prompts, onboarding flows, and program sequencing.

The visual design lens (Anna Charity voice): Every screen should feel intentional and warm. Would this layout feel calming or overwhelming to someone having a hard day? Is there enough breathing room? Does the color palette support the emotional tone of this exercise? Are the animations purposeful or decorative? Does this feel like a space someone wants to be in, or a form they have to fill out? Hold every visual decision to this standard — if it wouldn't pass in a Headspace release, it needs more craft.
...
- **Animation & Interaction toolkit**:
  - `framer-motion` v12 — primary animation (expand/collapse, stagger, scroll reveals, gestures, SVG path animation). Used in 66+ files.
  - `lottie-react` — Lottie animations for micro-interactions (completion celebrations, transitions, empty states, feedback moments). Lottie JSON files go in `public/lottie/`.
  - `@dnd-kit` — drag-and-drop (used in CardSort, BubbleSort)
  - `@use-gesture/react` — swipe, pinch, drag gestures for native-feeling mobile interactions
  - `d3` v7 — SVG data visualization (EmotionWheel, BodyMap, WheelChart, StakeholderMap, etc.)
  - `recharts` v3 — React charting (available, underused)
  - `canvas-confetti` — lightweight celebration confetti on exercise completion
  - `@radix-ui/react-tooltip`, `@radix-ui/react-accordion`, `@radix-ui/react-popover` — accessible UI primitives for tooltips, progressive disclosure, contextual help
- **Exercise primitives**: 24 interactive components in `src/components/exercises/primitives/`. When building or modernizing exercises, use these tools to create interactions that feel modern, fun, and easy to use. Prefer motion and gesture-based interactions over static forms. Use Lottie for delightful feedback moments. Use Radix for accessible progressive disclosure.
...
- **Design reference targets** (aspire to this level of craft):
  - **Headspace** — gold standard for wellness app design. Custom illustrations, warm palettes, Lottie animations that feel hand-drawn, every screen intentional. The design itself is therapeutic.
  - **Brilliant.org** — interactive diagrams that feel alive, dark mode with vibrant accents. The visualizations ARE the product. Closest model for "the interaction is the learning."
  - **Linear** — benchmark for UI polish. Subtle gradients, micro-animations on every state change, spatial depth. Everything responds instantly.
  - **Duolingo** — chunked micro-lessons, immediate feedback, celebration moments, color-coded progress. Proves learning UI can be fun without being patronizing.
  - **Calm** — atmospheric, minimal chrome, the design disappears so the content breathes.
  - **Arc Browser** — bold color choices, spatial design, animations that communicate meaning not decoration.
  - **Notion** — whitespace, typography hierarchy, blocks that snap together. Makes complexity feel simple.
  - **Woebot** — conversational CBT, exercises embedded in chat flow. Makes therapeutic frameworks feel conversational, not clinical.
  - **Brilliant.org + Headspace + Linear** is the sweet spot for Mindcraft: warm but precise, interactive but not overwhelming, delightful but not distracting.
