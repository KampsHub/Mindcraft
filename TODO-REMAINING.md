...
**Coach Dashboard (#5)** — ✅ Decisions made. Dashboard built.
- Coaches see only clients who accepted sharing (via `coach_clients` invite flow) ✅
- Coach notes from dashboard → show up in daily thread. **To build:** Add note input in expanded client card, save to `coach_notes` or similar, pull into daily-themes prompt.
- Flag for follow-up → not requested for now.

**User-Facing Analytics (#8)** — ✅ Decision made.
- Top patterns detected → surface in weekly insights
- Shifts (tone/language/theme changes) → surface in weekly insights
- Both are generated by the weekly-insights API and shown on the weekly review page. No separate analytics page needed — weekly insights IS the analytics view.

**Notification Preferences (#9) — Proposed toggles:**
Looking at all user-facing emails, here's the full list:
- **Toggleable:** Inactive reminders (2-day nudges), Day completion emails, Program updates
- **Always send:** Welcome email, Program complete (milestone), Coach notes (coach-initiated), Exit survey (churn feedback)
- **Future:** Weekly insights digest email (doesn't exist yet — would summarize the week's insights and send Saturday morning)
- [ ] Does this breakdown look right? Any email you'd add or remove from the toggleable list?
