...
**Notification Preferences (#9)** — ✅ Decision made.
- **Toggleable:** Inactive reminders (2-day nudges), Program updates
- **Always send:** Welcome, Program complete, Coach notes, Exit survey
- **Removed:** Day completion emails (not necessary)
- **Future:** Weekly insights digest (Saturday morning summary)
- **To build:** Add `email_preferences` jsonb column to `clients`, UI toggles in `/my-account`, check preferences in re-engage cron before sending.
