...
async function collectUserData(supabase: ReturnType<typeof createServerClient>, userId: string) {
  const [
    ...
    supabase.from("daily_sessions").select("id, day_number, step_2_journal, step_5_summary, day_rating, day_feedback, completed_at, enrollment_id").eq("enrollment_id", (await supabase.from("program_enrollments").select("id").eq("client_id", userId)).data?.map(e => e.id) || []),
