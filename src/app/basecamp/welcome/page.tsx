...
export default function BasecampWelcomePage() {
  return (
    <Suspense fallback={
...
function BasecampWelcome() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/checkout/verify?session_id=${sessionId}`)
      ...
      .then((data) => {
        ...
        if (data.paid) {
          trackEvent("new_role_purchase", { tier: data.tier || "standard", amount: data.amountPaid });
        }
      })
  ...
  async function createEnrollmentIfNeeded(userId: string, userEmail: string) {
    ...
    const { data: program } = await supabase.from("programs").select("id").eq("slug", "basecamp").single();
    if (!program) return;
      ...
      <div style={{ position: "relative", maxWidth: 480, margin: "0 auto", padding: "80px 24px 64px" }}>
        ...
        {verified === true && (
          <>
            ...
            >
              New roles are disorienting — even the ones you wanted. You just decided to navigate yours with intention. Create your account and start whenever you&rsquo;re ready.
            </motion.p>
            ...
            {!success ? (
              <motion.div {...fade(0.7)}>
                ...
                <motion.div {...fade(0.9)}>
                  ...
                  {[
                    "Answer 6 short questions so the program fits your new role.",
            ...
            >
              The first step is showing up with intention. You just took it.
            </motion.p>
          </>
        )}
      </div>
